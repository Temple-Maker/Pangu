...
- Round 3 easy sweep (three quick vec4 PRs, all bitwise-identical, same ``alignas``/float4 idiom as before):
  - ``softcap.cu`` — ``perf/softcap-float4``, PR #12. Fused SCALE+TANH+SCALE (Gemma-2-style attention softcapping) — the only warm one of the three. PDL preserved.
  - ``fill.cu`` — ``perf/fill-vec4``, PR #13. Store-only broadcast; also folded the per-type launch duplication into one templated launcher. Cold op, stated honestly in the PR.
